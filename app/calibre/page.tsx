...
const faqs=[
  ...
  ["We already use DISC or psychometrics.","Those describe personality. Calibre installs an evaluation method and interviewer calibration — it complements psychometrics, it doesn't compete with them."],
  ["Isn't assessing candidates a legal risk?","Calibre is about how your team evaluates, not a personality test on candidates. It's structured, job-relevant, and documented — built to make decisions more defensible, not less."],
  ["Our recruiters are experienced.","Experience isn't consistency. In the first session, the two-reviewers-one-CV exercise usually surfaces a multi-point gap inside your own team — live."],
];
const packages=[
  ["Diagnostic","The front door","A fixed-fee review of your funnel, interview process, and a sample of recent decisions — returned as a one-page findings sheet."],
  ["Calibrate","The workshop","A half- or full-day session: the evaluation framework, structured-interview design, scorecards, and question bank — so your team scores the same way."],
...
export default function Calibre(){return <main className="calibre">
  <header><Brand/><nav><a href="#method">Method</a><a href="#deliver">Deliverables</a><a href="#who">Who it&apos;s for</a><a href="#pilot">Pilot</a><a href="#faq">FAQ</a><Link href="/">Daftar</Link><Link href="/ar/calibre" className="lang" aria-label="العربية">ع</Link><a className="button small" href="#scope">Book a call</a></nav><a className="menu" href="#method" aria-label="Open navigation">☰</a></header>
  ...
  <section className="cred"><div className="wrap"><article><Eyebrow>Who builds it</Eyebrow><h3>Evaluation from the executive-search and Big-4 side of the table — taught to your team.</h3><p>Built by Ahmad Al Hinaiti, a senior practitioner in hiring quality across the region.</p></article><article><Eyebrow>Where it fits</Eyebrow><h3>Built for GCC hiring under localization pressure.</h3><p>Saudi-first (Nitaqat 2.0), then the UAE and the wider Gulf — fast hiring that still has to be defensible.</p></article></div></section>
  ...
  <section id="packages" className="section soft"><div className="wrap"><Eyebrow>How we work together</Eyebrow><h2>Three ways in, one <em>method</em>.</h2><div className="grid three">{packages.map((x,i)=><article className="card" key={x[0]}><code>{String(i+1).padStart(2,"0")}</code><h3>{x[0]}</h3><span className="tier">{x[1]}</span><p>{x[2]}</p></article>)}</div></div></section>
